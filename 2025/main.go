package main

import (
	"os"
	"strconv"

	"github.com/petkovicdanilo/advent-of-code-2025/day1"
	"github.com/petkovicdanilo/advent-of-code-2025/day2"
	"github.com/petkovicdanilo/advent-of-code-2025/day3"
	"github.com/petkovicdanilo/advent-of-code-2025/day4"
	"github.com/petkovicdanilo/advent-of-code-2025/day5"
	"github.com/petkovicdanilo/advent-of-code-2025/day6"
	"github.com/petkovicdanilo/advent-of-code-2025/day7"
	"github.com/petkovicdanilo/advent-of-code-2025/day8"
	"github.com/petkovicdanilo/advent-of-code-2025/day9"
	"github.com/petkovicdanilo/advent-of-code-2025/day10"
	"github.com/petkovicdanilo/advent-of-code-2025/day11"
	"github.com/petkovicdanilo/advent-of-code-2025/day12"
)

func main() {
	day, err := strconv.ParseUint(os.Args[1], 10, 8)
	if err != nil {
		panic(err)
	}

	part, err := strconv.ParseUint(os.Args[2], 10, 8)
	if err != nil {
		panic(err)
	}

	fileName := os.Args[3]


	switch day {
	case 1:
		switch part {
		case 1:
			must(day1.SolvePartOne(fileName))
		case 2:
			must(day1.SolvePartTwo(fileName))
		}
	case 2:
		switch part {
		case 1:
			must(day2.SolvePartOne(fileName))
		case 2:
			must(day2.SolvePartTwo(fileName))
		}
	case 3:
		switch part {
		case 1:
			must(day3.SolvePartOne(fileName))
		case 2:
			must(day3.SolvePartTwo(fileName))
		}
	case 4:
		switch part {
		case 1:
			must(day4.SolvePartOne(fileName))
		case 2:
			must(day4.SolvePartTwo(fileName))
		}
	case 5:
		switch part {
		case 1:
			must(day5.SolvePartOne(fileName))
		case 2:
			must(day5.SolvePartTwo(fileName))
		}
	case 6:
		switch part {
		case 1:
			must(day6.SolvePartOne(fileName))
		case 2:
			must(day6.SolvePartTwo(fileName))
		}
	case 7:
		switch part {
		case 1:
			must(day7.SolvePartOne(fileName))
		case 2:
			must(day7.SolvePartTwo(fileName))
		}
	case 8:
		switch part {
		case 1:
			must(day8.SolvePartOne(fileName))
		case 2:
			must(day8.SolvePartTwo(fileName))
		}
	case 9:
		switch part {
		case 1:
			must(day9.SolvePartOne(fileName))
		case 2:
			must(day9.SolvePartTwo(fileName))
		}
	case 10:
		switch part {
		case 1:
			must(day10.SolvePartOne(fileName))
		case 2:
			must(day10.SolvePartTwo(fileName))
		}
	case 11:
		switch part {
		case 1:
			must(day11.SolvePartOne(fileName))
		case 2:
			must(day11.SolvePartTwo(fileName))
		}
	case 12:
		switch part {
		case 1:
			must(day12.SolvePartOne(fileName))
		}
	}
}


func must(err error) {
    if err != nil {
        panic(err)
    }
}
